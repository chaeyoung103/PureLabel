...
class SignupViewController: UIViewController {
    ...
    @objc func nextBtnDidTab() {
        let props = Props(name: name.text ?? "", id: id.text ?? "", password: password.text ?? "")
        let signUpViewController2 = SignupViewController2(data: props)
    ...
    private func updateLoginButtonState() {
        ...
        nextBtn.isEnabled = !isNameEmpty && !isEmailEmpty && !isPasswordEmpty && !isPasswordCheckEmpty
        
        if nextBtn.isEnabled {
            nextBtn.backgroundColor = .buttonBgColor
        } else {
            nextBtn.backgroundColor = .buttonBgColor_inactive
        }
    }
